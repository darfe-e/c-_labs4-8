...
std::istream& operator>> (std::istream& is, IndependentPublicationCard& card)
{
    ...
    std::cout << "Введите издательство: ";              // Приглашение для ввода названия издательства
    std::getline(is, card.publisher);            // Чтение всей строки названия издательства

    std::cout << "Введите год издания: ";               // Приглашение для ввода года издания
    is >> card.yearOfPublication;                       // Чтение целочисленного значения года издания
    is.ignore();                                        // Очистка буфера ввода после чтения числа

    std::cout << "Введите тираж: ";                     // Приглашение для ввода тиража
    is >> card.circulation;                             // Чтение целочисленного значения тиража
    is.ignore();                                        // Очистка буфера ввода после чтения числа

    std::cout << "Введите количество страниц: ";        // Приглашение для ввода количества страниц
    is >> card.pagesNamber;                             // Чтение целочисленного значения количества страниц
    is.ignore();                                        // Очистка буфера ввода после чтения числа
